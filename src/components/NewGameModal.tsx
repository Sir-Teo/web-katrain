...
export const NewGameModal: React.FC<NewGameModalProps> = ({
  ...
  const timerSummary = timerConfig.mode === 'none'
    ? 'No timer'
    : `${timerConfig.mainTimeMinutes > 0 ? `${timerConfig.mainTimeMinutes} min + ` : ''}${timerConfig.byoPeriods} × ${timerConfig.byoLengthSeconds}s`;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      ...
      >
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1">
              ...
              <div className="text-[0.6875rem] ui-text-faint">
                {handicap > 0 ? 'Placed on star points; White plays first.' : 'Black plays first.'}
              </div>
            </div>
          </div>
        ...
        <div className="new-game-modal-footer px-4 py-3 border-t border-[var(--ui-border)] flex justify-end gap-2 ui-bar">
          ...
          <button type="button"
            className="px-3 py-2 rounded ui-accent-bg not-disabled:hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed"
            disabled={komi === null}
