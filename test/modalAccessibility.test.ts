...
describe('modal accessibility semantics', () => {
  ...
  it('moves focus into dialogs that use the shared focus hook and wraps Tab inside them', () => {
    ...
    // mountOpener is state set once at mount, so it never re-runs the effect.
    expect(hook).toContain('}, [active, focusContainer, initialFocusRef, mountOpener, returnFocus]);');
