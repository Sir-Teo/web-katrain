...
manual-scoring integration fixes, `npm run verify` passed 2,620 tests, with the same skip,
...
| Medium | Game Info's editable rule picker is incomplete. | The display label handles all seven supported rules, but the editor lists only Japanese, Chinese, and Korean. Native selection behavior for imported AGA, New Zealand, Tromp–Taylor, and Ancient Chinese games needs a regression, followed by sharing the central rules list. |
