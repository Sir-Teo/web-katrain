...
the deferred-cancellation follow-up, `npm run verify` passed 2,459 tests, with the same skip,
...
| Medium | Engine status waits for the first completed search. | A production browser reproduction shows a live evaluation alongside “Loading model”: progress updates synchronize metadata inside the client, but the store copies it only on the final result. Update readiness on valid progress, while preserving cancellation and model-change guards. |
