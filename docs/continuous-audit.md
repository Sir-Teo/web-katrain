...
search self-capture fixes, `npm run verify` passed 2,553 tests, with the same skip,
...
| High | Professional engine validation needs broader evidence. | The bundled model comes from KataGo's test fixtures. Existing golden/invariant tests are useful but do not prove b18 search equivalence across all supported rules and endgames. Root/deeper-search self-capture and post-move replay now have direct regressions. Next inspect selected-rule area features and full-history superko, then add reference positions for stronger weights, seki, pass handling, and handicap compensation. Rule-input flags alone do not establish search parity. |
