...
search self-capture and area-feature fixes, `npm run verify` passed 2,570 tests, with the same skip,
...
| High | Professional engine validation needs broader evidence. | The bundled model comes from KataGo's test fixtures. Existing golden/invariant tests are useful but do not prove b18 search equivalence across all supported rules and endgames. Root/deeper-search self-capture, post-move replay, and selected-rule area inputs now have direct regressions. Next inspect full-history superko and endgame paths: root pass-alive pruning and terminal area calculation still use the default no-suicide flag, while ending bonuses and friendly-pass suppression special-case Chinese. Establish reference failures before changing these paths. Also validate stronger weights, model-specific history modes, seki, and handicap compensation. |
