...
terminal area-scoring fixes, `npm run verify` passed 2,608 tests, with the same skip,
...
| --- | --- | --- |
| High | The manual Score panel ignores the selected rules. | `Layout` calls `computeManualScoreEstimate` without rules or handicap count, so it defaults to Japanese territory scoring. On a settled area-rules position, all five rulesets showed W+9 instead of B+14/B+16 at 1280×800 and 390×844; Done recorded W+9 in SGF `RE`. Fix the calculation inputs, rule-dependent breakdown, and result-recording workflow next. |
