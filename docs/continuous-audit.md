...
search-history and endgame-pruning fixes, `npm run verify` passed 2,582 tests, with the same skip,
...
| High | Professional engine validation needs broader evidence. | The bundled model comes from KataGo's test fixtures. Existing golden/invariant tests are useful but do not prove b18 search equivalence across all supported rules and endgames. Self-capture, area inputs, search-history transport, and root pass-alive pruning now have direct regressions. Next inspect full-history superko and remaining endgame paths: terminal area calculation still uses the default no-suicide flag, while ending bonuses and friendly-pass suppression special-case Chinese. Establish reference failures before changing these paths. Also validate stronger weights, model-specific history modes, seki, and handicap compensation. |
